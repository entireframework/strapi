...
describe('appendSearchParamsToUrl', () => {
  ...
  test('returns original string with no update time', () => {
    ...
    expect(appendSearchParamsToUrl({ url: urlString, params: { updatedAt: undefined } })).toEqual(
      urlString
    );
  });
  ...
  test('appends update time to string with no search params', () => {
    ...
    expect(appendSearchParamsToUrl({ url: urlString, params: { updatedAt: updateTime } })).toEqual(
      expected
    );
  });
  ...
  test('appends update time to string with search params', () => {
    ...
    expect(
      appendSearchParamsToUrl({ url: urlWithQuery, params: { updatedAt: updateTime } })
    ).toEqual(expected);
  });
  ...
  test('appends multiple search params', () => {
    ...
    expect(appendSearchParamsToUrl({ url: urlString, params: { param1, param2 } })).toEqual(
      expected
    );
  });
});
